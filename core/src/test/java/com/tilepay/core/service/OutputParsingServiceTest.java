...
public class OutputParsingServiceTest {
    ...
    public void parseCounterPartyOutput() {
        ...
        Mockito.when(assetConverter.getAsset("TILECOINX")).thenReturn(AssetBuilder.anAsset().setName("TILECOINX").setDivisible(true).build());

        outputParsingService.parseAssetOutput(data, output);

        assertEquals("TILECOINX", output.getAsset().getName());
